# Image Conversion Tracker - 画像変換追跡システム
...
**Image Conversion Tracker**は、ノードベースのビジュアルワークフローエディタを使用した画像処理パイプライン構築ツールです。
...
#### 6. **Undo/Redoと履歴スナップショット** ⭐ 簡単
- **実装時間**: 1〜2時間
- **目的**: 直前の操作を取り消したりやり直したりできるようにし、誤操作時の復元を容易にする
- **実装内容**:
  - ノード追加/削除、エッジ接続、レイアウト変更などの操作を履歴スタックに積む
  - Ctrl+Z/Ctrl+YショートカットとツールバーのUndo/Redoボタンを追加
  - ネットワーク状態をJSONスナップショットとしてlocalStorageに世代管理（例: 最新20件）し、復元はスナップショット単位で行う
- **実装箇所**: `js/canvasController.js`、`js/workflowEngine.js`、`js/workflowApp.js`
